...
class MonoTextData(object):
    ...
    def read_corpus_form_input(self,  batch_first, device, max_length=None):
        data = []
        vocab = self.vocab
        dropped = 0
        a = input("input1:")
        b = input("input2:")
        lines=[a.lower(),b.lower()]
        ...
        for line in lines:
            ...
            if max_length:
                if len(split_line) > max_length:
                    dropped += 1
                    continue
            d= [vocab[word] for word in split_line if word in vocab]
            d,_ = self._to_tensor(batch_data=[d], device=device, batch_first=batch_first)
            data.append(d)
